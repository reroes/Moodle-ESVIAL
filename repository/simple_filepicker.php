...
$home_url = new moodle_url('/repository/simple_draftfiles_manager.php', $params);

$params['savepath'] = $savepath;
$params['repo_id'] = $repo_id;
$url = new moodle_url($CFG->httpswwwroot."/repository/simple_filepicker.php", $params);

$PAGE->set_url('/repository/simple_filepicker.php', $params);
    ...
    if ($repo->check_login()) {
        ...
        if (!empty($list['upload'])) {
            ...
            if (!empty($list['page'])) {
                ...
                $pagingurl = new moodle_url("$CFG->httpswwwroot/repository/simple_filepicker.php?action=list&itemid=$itemid&ctx_id=$contextid&repo_id=$repo_id&course=$courseid&sesskey=".  sesskey());
...
default:
    echo $OUTPUT->header();
    echo 'Hubo un error al querer subir el archivo<br>';
    echo '<div><a href="'.$home_url->out().'">'.get_string('back', 'repository').'</a></div>';
    break;
}
