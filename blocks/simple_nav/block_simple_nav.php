...
class block_simple_nav extends block_base {
	...
	function simple_nav_get_class_if_active ($myid, $mytype) {
		...
		if ($mytype == 'user-edit' && ($PAGE->pagetype == 'user-edit')) {
			$myclass = ' active_tree_node';
			return $myclass;
		}

		if ($mytype == 'login-change_password' && ($PAGE->pagetype == 'login-change_password')) {
			$myclass = ' active_tree_node';
			return $myclass;
		}

		if ($mytype == 'message-edit' && ($PAGE->pagetype == 'message-edit')) {
			$myclass = ' active_tree_node';
			return $myclass;
		}

		if ($mytype == "user-profile" && ($PAGE->pagetype == 'user-profile')) {
			$myclass = ' active_tree_node';
			return $myclass;
		}
	...
	function get_content() {
		...
		if (! empty($this->config->sn_profile)) {
			$sn_profile = $this->config->sn_profile;
		} else {
			$sn_profile = get_string('profile');
		}
		...
		$check_startcategory ="startcategory_profile";
		if (isset($this->config->$check_startcategory)) {
			$startcategory_profile = $this->config->$check_startcategory;
		}
		else {
			$startcategory_profile = 1;
		}
		if ($startcategory_profile == 1 && $USER->id <> 0) {
			$myclass = $this->simple_nav_get_class_if_active(null, 'user-profile');
			$items[]=$this->simple_nav_collect_items($myclass, null, $sn_profile, null, 'user-profile', 0, null, null);
			$myclass = $this->simple_nav_get_class_if_active(null, 'user-edit');
			$items[]=$this->simple_nav_collect_items($myclass, null, 'Editar Perfil', null, 'user-edit', 0, null, null);
			$myclass = $this->simple_nav_get_class_if_active(null, 'login-change_password');
			$items[]=$this->simple_nav_collect_items($myclass, null, 'Cambiar Contraseña', null, 'login-change_password', 0, null, null);
			$myclass = $this->simple_nav_get_class_if_active(null, 'message-edit');
			$items[]=$this->simple_nav_collect_items($myclass, null, 'Mensajería', null, 'message-edit', 0, null, null);
		}
		else {
			// if we don't use "profile" we still print it, but don't use text. This is important to not destroy the html and javaScript
			//$myclass = $this->simple_nav_get_class_if_active(null, null);
			//$items[]=$this->simple_nav_collect_items($myclass, null, $sn_profile, null, 'noprofile', 0, null, null);
		}
		...
		foreach ($categories as $category) {
			...
			if ($startcategory_value >= 1) {
				...
				$items_2[]=$this->simple_nav_collect_items($myclass, $category->id, $category_name, $category->depth+1, 'category', $category->id, $icon, 1);
			}
			else {
				continue;
			}	
			...
			foreach ($courses as $course) {
			   ...
			   foreach ($roles as $role) {
				...
				if ($category->id == $course->category && $show_courses ) {
					...
					$items_3[]=$this->simple_nav_collect_items ($myclass, $course->id, '('.$course->shortname.') '. $course->fullname, $category->depth+2, 'course', $category->id, $icon, $course->visible);
					...
					foreach ($modules as $module) {						
						...
						foreach ($module_items as $module_item) {
							if ($module_item['name'] == $module_object->modname && $module_item['value']) {
								...
								$items_3[]=$this->simple_nav_collect_items ($myclass, $module_object->id, $module_object->name, $category->depth+3, 'module', $module_object->course, $module_object->modname, $course->visible);
								break;
							}
						}
		   ...
		   if (count($items_3)>0) {
			...
			$items = array_merge($items, $items_3);
		   }
		   //This is used to show only the categories and courses on which the current user has been assigned.
		   $items_3 = array();
