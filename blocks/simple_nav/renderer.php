...
class block_simple_nav_renderer extends plugin_renderer_base {
    ...
    public function simple_nav_tree($items) {
		...
		if (($item['mytype'] == 'category' && $type == 'course') || ($item['mytype'] == 'category' && $type == 'message-edit')) {
			$content[] = '</ul>';
		}

		if ($type == 'category' && $item['mytype'] == 'course') {
			$content[] = '<ul>';
		}
    ...
    protected function sn_print_item($myclass, $myid, $myname, $mydepth, $mytype, $mypath, $myicon, $myvisibility) {
		...
		elseif ($mytype == 'course') {
			...
			$myclass_p = 'tree_item branch hasicon'.$myclass.' navigation_node';
			$myopentag = '';
		...
		elseif ($mytype == 'home') {
			...
			$myclass_li = 'type_unknown depth_1 '.$mycollapsed.' contains_branch';
		...
		elseif ($mytype == 'nohome') {
			$myurl =$CFG->wwwroot;
			$myclass_ul_open = '<ul class="block_tree list">';
			$myclass_li = 'type_unknown depth_1 '.$mycollapsed.' contains_branch simple_invisible';
			$myclass_p = 'tree_item branch '.$myclass.' navigation_node';
			$myopentag = '';
			$myclass_a = '';

		}
		elseif ($mytype == 'user-profile') {
			if ($USER->id <> 0) {
			   $myurl =$CFG->wwwroot.'/user/profile.php?id='.$USER->id;
			   $myclass_ul_open = '';
			   $myclass_li = 'type_profile depth_2'.$mycollapsed.' contains_branch';
			   $myclass_p = 'tree_item branch hasicon '.$myclass;
			   $myopentag = '<ul>';
			   $myclass_a = '';
			}
		}
		elseif ($mytype == 'user-edit') {
			$myurl =$CFG->wwwroot.'/user/edit.php?id='.$USER->id;
			$myclass_ul_open = '';
		...
		elseif ($mytype == 'message-edit') {
			$myurl =$CFG->wwwroot.'/message/edit.php?id='.$USER->id;
			$myclass_ul_open = '';
			$myclass_li = 'type_profile depth_1'.$mycollapsed.' contains_branch';
			$myclass_p = 'tree_item branch '.$myclass.' navigation_node';
			$myopentag = '';
		...
		// or invisible profile node
		elseif ($mytype == 'noprofile') {
			if ($USER->id <> 0) {
			   $myurl =$CFG->wwwroot.'/user/profile.php?id='.$USER->id;
			   $myclass_ul_open = '<ul class="block_tree list">';
			   $myclass_li = 'type_profile depth_2 contains_branch';
			   $myclass_p = 'tree_item branch hasicon '.$myclass;
			   $myopentag = '';
			   $myclass_a = '';
			}
		...
		elseif ($mytype == 'module') {
			...
			$icon = '<img alt="*" class="smallicon navicon" title="'.$myicon.'" src="'.$baseurl.'/theme/image.php?theme=standard&amp;image=icon&amp;rev=295&amp;component='.$myicon.'">';
		}
		
		$myitem = $myclass_ul_open.'<li class="'.$myclass_li.'"><p class="'.$myclass_p.'"><a '.$myclass_a.' href="'.$myurl.'">'.$icon.''.$myname.'</a></p>'.$myopentag;
		if ($mytype == 'notifications') {
			$myitem .= '</ul>';
		}
		return $myitem;
	}
